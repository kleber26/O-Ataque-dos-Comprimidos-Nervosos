...
public class vThirdPersonCamera : MonoBehaviour
{
    ...
    private float waitAimingTimer = 0f;
    ...
    public void AimingCamera(bool aim) {
        
//        if (aim && waitAimingTimer < 8f) {
//            waitAimingTimer += 1f;
//            return;
//        }
//
...
//        
        ...
        if (aim && Time.time >= nextTimetoFire) {
            ...
            height = 1.66f;
            crosshair.active = true;
            nextTimetoFire = Time.time + 3f / 10f;
        } else if (!aim && Time.time >= nextTimetoFire){
            rightOffset = 0f;
            defaultDistance = 2.5f;
            height = 1.4f;
            waitAimingTimer = 0f;
